...
--○ 정규화란?
--  한 마디로 데이터 베이스 서버의 메모리 낭비를 막기 위해
...
-- 테이블명 : 직원 → 자식 테이블
/*
   10Byte  10Byte10Byte      10Byte           10Byte 
-----------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
   ...
   AHR9238(안혜리)    PKKBC(꼬북칩)          2020-10-02 14:03:15            20
   KJH5266(김종호)    PKKDS(쿠크다스)        2020-10-02 14:15:30            13
   KSY5266(권소윤)    PSWKK(새우깡)          2020-10-03 09:00:11             5
                        :
                        :
--------------------------------------------------------------------------------
*/
...
--   컬럼 한 개로만 구성된 PRIMARY KEY를 SINGLE PRIMARY KEY라고 부른다.
--   (단일 프라이머리 키)
...
O3309       22      2018318(윤홍준)     80
O3309       22      2018320(이예슬)     96
                    :
---------------------------------------------------------------------------------------
*/
...
1. 관계(relationship, relation)
    ...
    - 각 열(column)은 유일한 이름을 가지며 순서는 무의미 하다.
...
HR	SYS_C007077	P	TBL_TEST1					ENABLED	NOT DEFERRABLE	IMMEDIATE	VALIDATED	GENERATED NAME			2020-10-08	HR	SYS_C007077		
*/
...
WHERE TABLE_NAME = 'TBL_TEST1';
--==>> HR	SYS_C007077	TBL_TEST1	COL1	1
...
HR	SYS_C007077	TBL_TEST1	P	COL1
*/
